...
import { auth, users as usersApi, chats as chatsApi, posts as postsApi, stories as storiesApi, videos as videosApi, music as musicApi, notifications as notifApi, admin as adminApi, search as searchApi } from './store';
    ...
    <div className="h-full flex items-center justify-center" style={{ background: '#000' }}>
      {storyList.length === 0 ? (
        ...
        <div className="relative w-full max-w-sm h-full max-h-[85vh] rounded-3xl overflow-hidden">
          ...
          {storyList[0]?.text_content && (
            <div className="absolute bottom-20 left-4 right-4">
              <p className="text-white text-lg font-medium text-center">{storyList[0].text_content}</p>
            </div>
          )}
        </div>
      )}
